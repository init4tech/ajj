...
if ! cargo clippy -p ajj --all-targets --all-features -- -D warnings 2>&1; then
    echo "Clippy (--all-features) failed." >&2
    exit 2
fi

if ! cargo clippy -p ajj --all-targets --no-default-features -- -D warnings 2>&1; then
    echo "Clippy (--no-default-features) failed." >&2
    exit 2
fi

if ! RUSTDOCFLAGS="-D warnings" cargo doc -p ajj --no-deps 2>&1; then
